...
export default (
  {
    ...
    skipGuild,
    changeIsOpen,
    onChangeIsOpen = ((e) => { changeIsOpen(e.target.checked); }),
  ...
  if (SceneMsg.apply_img && SceneMsg.apply_img !== undefined) {
    img = SceneMsg.apply_img;
    isloding = false;
  } else {
    ...
    <div className={styles.root}>
      ...
      <div className={styles.footer}>
        <div className={`${styles.btnBox} ${styles.rightBtn}`}>
          <Button className={styles.btn} onClick={() => step4(-1)}>上一步</Button>
        ...
        <div className={styles.jumpStep}>
          <Checkbox data-analyze={[3, 2]} onChange={onChangeIsOpen}>下次不再显示</Checkbox>
        </div>
      </div>
    </div>
  );
